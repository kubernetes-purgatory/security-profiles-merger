...
func TestGlobMatchingSyntax(t *testing.T) {
	...
	tests := []struct {
		...
		{"bang negated class", "/lib/[!a].so", "/lib/b.so", true},
		...
		{"class with bracket member", "/lib/[]a].so", "/lib/].so", true},
		{"class with dash member", "/lib/[a-].so", "/lib/-.so", true},
		...
		{"escaped plain character", `/etc/\a*`, "/etc/abc", true},
